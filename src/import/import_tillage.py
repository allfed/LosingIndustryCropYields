...
import src.utilities.params as params
import src.utilities.plotter as Plotter

# from src.utilities.utilities import utilities
# from src.utilities import (
#     params,
# )  # get file location and varname parameters for data import
...
for c in crops:
    ...
    for m in ["mech", "non_mech"]:
        ...
        result[
            start_lat_index : start_lat_index + len(areas),
            start_lon_index : start_lon_index + len(areas[0]),
        ] = areas
    ...
    if MAKE_GRID:
        ...
        grid[c + "_is_mech"] = np.where(
            grid[c + "_no_crops"], np.nan, grid[c + "_is_mech_tmp"]
        )
        ...
        Plotter.plotMap(
            grid, c + "_is_mech", title, label, "TillageMechWheat", plotGrowArea
        )
    else:
        ...
        df[c + "_is_mech"] = np.where(
            df[c + "_no_crops"], np.nan, df[c + "_is_mech_tmp"]
        )
    ...
    if not MAKE_GRID:
        if len(allcrops) == 0:
            ...
            allcrops["is_mech"] = (
                df[c + "_is_mech"].where(df[c + "_is_mech"] == 1, 0).astype("bool")
            )
        else:
            ...
            allcrops["is_mech"] = allcrops["is_mech"] | df[c + "_is_mech"].where(
                df[c + "_is_mech"] == 1, 0
            ).astype("bool")
        df[c + "_is_mech"] = np.where(
            np.isnan(df[c + "_is_mech"]), -9, df[c + "_is_mech"]
        )
