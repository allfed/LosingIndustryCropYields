...
rice_yield = pd.read_pickle(
    params.inputDataDir + "RICECropYieldHighRes.pkl", compression="zip"
)
...
r_pesticides = pd.read_pickle(
    params.inputDataDir + "RicePesticidesHighRes.pkl", compression="zip"
)
...
dr0_qt = dr0_raw.quantile([0.25, 0.5, 0.75, 0.85, 0.95, 0.99, 0.999])
...
# n_manure values
dr0_elim.loc[dr0_elim["n_total"] < 0, "n_total"] = (
    dr0_elim["n_fertilizer"] + dr0_elim["n_manure"]
)

# calculate the 25th, 50th, 75th, 85th, 95th, 99th and 99.9th percentile
dr0_qt = dr0_elim.quantile([0.25, 0.5, 0.75, 0.85, 0.95, 0.99, 0.999])
dr0_qt.reset_index(inplace=True, drop=True)
...
#####Rename Columns##########
dur_mst_elim = dur_mst_elim.rename(
    columns={
        1: "LGP<120days",
        3: "120-180days",
        4: "180-225days",
...
dur_thz_elim = dur_thz_elim.rename(
    columns={
        1: "Trop_low",
        2: "Trop_high",
        3: "Sub-trop_warm",
...
dur_soil_elim = dur_soil_elim.rename(
    columns={
        1: "S1_very_steep",
        2: "S2_hydro_soil",
        3: "S3_no-slight_lim",
...
drice_d_elim = pd.concat(
    [dr0_elim, dur_mst_elim, dur_thz_elim, dur_soil_elim], axis="columns"
)
# drop one column of each dummy (this value will be encoded by 0 in all columns)
drice_dur_elim = drice_d_elim.drop(
...
drice_cor_elim = drice_fit_elim.drop(
    [
        "lat",
        "lon",
        "area",
...
np.sqrt(
    mean_tweedie_deviance(drice_fit_elim["Y"], r_fit_elimg.fittedvalues, power=2)
)  # 0.4954
...
plot_rs.axes[0] = sb.regplot(
    "Fitted_link",
    "resid_stud",
    data=r_elimg_infl_sample,
    lowess=True,
...
for i in abs_resid_top_3.index:
    plot_rr.axes[0].annotate(
        i, xy=(r_elimg_infl["GLM_fitted"][i], r_elimg_infl["resid_resp_abs"][i])
    )
...
for r, i in enumerate(abs_norm_resid_top_3):
    plot_rq.axes[0].annotate(
        i, xy=(np.flip(QQ.theoretical_quantiles, 0)[r], r_elimg_infl["resid_stud"][i])
    )
...
r_cook_infl = r_elimg_cook.loc[r_elimg_cook > (4 / (75194 - 21))].sort_values(
    ascending=False
)
...
sb.regplot(
    r_elimg_infl["hat_matrix"],
    r_elimg_infl["resid_stud"],
    scatter=False,
    ci=False,
    # lowess=True,
    line_kws={"color": "red", "lw": 1, "alpha": 0.8},
...
for i in leverage_top_3:
    plot_rc.axes[0].annotate(
        i, xy=(r_elimg_infl["hat_matrix"][i], r_elimg_infl["resid_stud"][i])
    )
...
rn_drop = LoI_relim[
    ((LoI_relim["mechanized"].isna()) | (LoI_relim["pesticides_H"].isna()))
    & (LoI_relim["n_fertilizer"] < 0)
].index
...
LoI_relim_pn[["n_fertilizer", "p_fertilizer"]] = LoI_relim_pn[
    ["n_fertilizer", "p_fertilizer"]
].fillna(method="ffill")
# replace no data values in n_total with the sum of the newly filled n_fertilizer and the
# n_manure values
LoI_relim_pn.loc[LoI_relim_pn["n_total"] < 0, "n_total"] = (
    LoI_relim_pn["n_fertilizer"] + LoI_relim_pn["n_manure"]
)
...
LoI_relim_pn["n_fert_y1"] = (r_ntot_new * LoI_relim_pn["n_ffrac"]) / LoI_relim_pn[
    "area"
]
...
# the distribution remains the same as before the catastrophe
LoI_relim_pn["p_fert_y1"] = (r_ptot_new * LoI_relim_pn["p_ffrac"]) / LoI_relim_pn[
    "area"
]
...
LoI_relim = pd.concat(
    [
        LoI_relim,
        LoI_relim_pn["n_fert_y1"],
        LoI_relim_pn["n_fert_y2"],
        LoI_relim_pn["p_fert_y1"],
        LoI_relim_pn["p_fert_y2"],
        LoI_relim_man["man_fert"],
        LoI_relimp["pest_y1"],
        LoI_relimp["pest_y2"],
    ],
    axis="columns",
...
LoI_r_phase1 = LoI_r_phase1.rename(
    columns={
...
rmean_y1_weigh = round(
    np.average(r_yield_y1, weights=LoI_relim["area"]), 2
)  # 3148.82kg/ha
...
# rename the columns according to the names used in the model formular
LoI_r_phase2 = LoI_r_phase2.rename(
    columns={
...
# calculate weighted mean, min and max of predicted yield (1) including postive change rates
rmean_y2_weigh = round(
    np.average(r_yield_y2, weights=LoI_relim["area"]), 2
)  # 2955.57kg/ha
...
LoI_rice = LoI_rice.rename(
    columns={0: "r_yield_y1", 1: "r_y1_change", 2: "r_yield_y2", 3: "r_y2_change"},
    errors="raise",
)
...
utilities.create5minASCIIneg(
    LoI_rice, "r_y1_change", params.asciiDir + "LoIRiceYieldChange_y1"
)
utilities.create5minASCIIneg(
    LoI_rice, "r_yield_y1", params.asciiDir + "LoIRiceYield_y1"
)
utilities.create5minASCIIneg(
    LoI_rice, "r_y2_change", params.asciiDir + "LoIRiceYieldChange_y2"
)
utilities.create5minASCIIneg(
    LoI_rice, "r_yield_y2", params.asciiDir + "LoIRiceYield_y2"
)
utilities.create5minASCIIneg(
    LoI_rice, "r_y1_c0", params.asciiDir + "LoIRiceYieldChange_0y1"
)
utilities.create5minASCIIneg(
    LoI_rice, "r_y2_c0", params.asciiDir + "LoIRiceYieldChange_0y2"
)
