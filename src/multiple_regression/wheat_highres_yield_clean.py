...
import src.utilities.utilities as utilities  # get file location and varname parameters
...
wheat_yield = pd.read_pickle(
    params.inputDataDir + "WHEACropYieldHighRes.pkl", compression="zip"
)
...
w_pesticides = pd.read_pickle(
    params.inputDataDir + "WheatPesticidesHighRes.pkl", compression="zip"
)
...
dw0_elim.loc[
    dw0_elim["n_fertilizer"] < 0, "n_fertilizer"
...
np.sqrt(
    mean_tweedie_deviance(dwheat_fit_elim["Y"], w_fit_elimg.fittedvalues, power=2)
)  # 0.5194
...
wn_drop = LoI_welim[
    ((LoI_welim["mechanized"].isna()) | (LoI_welim["pesticides_H"].isna()))
    & (LoI_welim["n_fertilizer"] < 0)
].index
...
# replace no data values in n_total with the sum of the newly filled n_fertilizer and the
# n_manure values
LoI_welim_pn.loc[LoI_welim_pn["n_total"] < 0, "n_total"] = (
    LoI_welim_pn["n_fertilizer"] + LoI_welim_pn["n_manure"]
)
...
LoI_welim_pn["n_fert_y1"] = (w_ntot_new * LoI_welim_pn["n_ffrac"]) / LoI_welim_pn[
    "area"
]
...
# the distribution remains the same as before the catastrophe
LoI_welim_pn["p_fert_y1"] = (w_ptot_new * LoI_welim_pn["p_ffrac"]) / LoI_welim_pn[
    "area"
]
...
LoI_welim = pd.concat(
    [
        ...
        LoI_welim_pn["n_fert_y2"],
        LoI_welim_pn["p_fert_y1"],
        LoI_welim_pn["p_fert_y2"],
...
wmean_y1_weigh = round(
    np.average(w_yield_y1, weights=LoI_welim["area"]), 2
)  # 2252.36kg/ha
