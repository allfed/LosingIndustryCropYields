...
maize_yield = pd.read_csv(
    params.geopandasDataDir + CROP + "CropYield" + postfix + "Filtered.csv"
)
...
m_pesticides = pd.read_csv(
    params.geopandasDataDir + "CornPesticides" + postfix + "Filtered.csv"
)
...
dm0_elim.loc[
    dm0_elim["n_fertilizer"] < 0, "n_fertilizer"
...
for scale in np.arange(1, 500, 5):
    ...
    dmaize_val_elim["sumproduct_pred"] = (
        dmaize_val_elim["predicted"] * dmaize_val_elim["area"]
    )
    ...
    newlist["sumproduct"] = dmaize_val_elim.groupby(
        ["latbin", "lonbin"]
    ).sumproduct.sum()
    newlist["sumproduct_pred"] = dmaize_val_elim.groupby(
        ["latbin", "lonbin"]
    ).sumproduct_pred.sum()
...
# replace remaining no data values in the fertilizer datasets with NaN and then fill them
LoI_melim_pn.loc[
    LoI_melim_pn["n_fertilizer"] < 0, "n_fertilizer"
] = np.nan  # only 2304 left, so ffill
...
LoI_melim_pn = LoI_melim_pn.loc[
    LoI_melim_pn["n_fertilizer"] < dm0_qt.iloc[12, 4]
]  # ~180
LoI_melim_pn = LoI_melim_pn.loc[
    LoI_melim_pn["p_fertilizer"] < dm0_qt.iloc[12, 5]
]  # ~34
...
# the distribution remains the same as before the catastrophe
LoI_melim_man["man_fert"] = (m_man_av * LoI_melim_man["n_mfrac"]) / LoI_melim_man[
    "area"
]
...
if IRRIGATED:
    LoI_melim["irr_LoI"] = LoI_melim["irrigation_tot"] * (
        1 - LoI_melim["irrigation_rel"]
    )
...
if IRRIGATED:
    ...
    LoI_m_year1 = LoI_m_year1.rename(
        columns={
...
LoI_m_year2 = LoI_melim.iloc[
    :, independent_column_indices
]  # reorder the columns according to the order in dm0_elim
if IRRIGATED:
    LoI_m_year2 = LoI_m_year2[
        [
            "p_fert_y2",
            "man_fert",
            "pest_y2",
    ...
    # rename the columns according to the names used in the model formular
    LoI_m_year2 = LoI_m_year2.rename(
        columns={
...
LoI_maize.to_csv(
    params.geopandasDataDir + "LoI" + CROP + "Yield" + postfix + "Filtered.csv"
)
print("saved")
