...
dm0_qt = dm0_elim.quantile([ .25, .5, .75, .85, .95, .99, .999])
...
m_out = pd.concat([Y_out['Y'], nf_out['n_fertilizer'], pf_out['p_fertilizer'],
...
#Nullmodel
m_mod_elim0 = smf.glm(formula='Y ~ 1', data=dmaize_fit_elim, family=sm.families.Gamma(link=sm.families.links.inverse_power))
...
np.sqrt(mean_tweedie_deviance(dmaize_fit_elim['Y'], m_fit_elimg.fittedvalues, power=2)) #0.5968
...
plot_mr.axes[0] = sb.residplot('GLM_fitted', 'Yield', data=m_elimg_infl, 
                          #lowess=True, 
...
m_cook_infl = m_elimg_cook.loc[m_elimg_cook < (4/(156289-21))].sort_values(ascending=False)
...
LoI_melim_man['labor'] = LoI_melim_man['area']/5 #current value (7.4) is taken from Cole et al. (2016)
...
LoI_m_phase1 = LoI_m_phase1[['p_fert_y1', 'N_toty1', 'pest_y1', 'mechanized', 
                       'irr_LoI', 'thz_class', 'mst_class', 'soil_class']]
#rename the columns according to the names used in the model formular
LoI_m_phase1 = LoI_m_phase1.rename(columns={'p_fert_y1':"p_fertilizer", 'N_toty1':"n_total", 
...
mmean_y1_weigh = round(np.average(m_yield_y1, weights=LoI_melim['area']),2) #4412.24kg/ha
...
#change rate
mmean_y1c_weigh = round(np.average(m_y1_change, weights=mw),2) #-0.14 (~-14%) -0.2 (~-20%)
...
#reorder the columns according to the order in dm0_elim
LoI_m_phase2 = LoI_m_phase2[['p_fert_y2', 'man_fert', 'pest_y2', 'mechanized_y2', 
                       'irr_LoI', 'thz_class', 'mst_class', 'soil_class']]
#rename the columns according to the names used in the model formular
LoI_m_phase2 = LoI_m_phase2.rename(columns={'p_fert_y2':"p_fertilizer", 'man_fert':"n_total", 
...
#calculate weighted mean, min and max of predicted yield (1) including postive change rates
mmean_y2_weigh = round(np.average(m_yield_y2, weights=LoI_melim['area']),2) #3300.81kg/ha
...
mmean_y2c_weigh = round(np.average(m_y2_change, weights=mw),2) #-0.35 (~-35%) -0.4 (~-40%)
...
print(dm0_mean, m_fit_mean, mmean_y1_weigh, mmean_y2_weigh,
      dm0_prod, m_fit_prod, m_y1_prod, m_y2_prod, 
      dm0_max, m_fit_max, mmax_y1, mmax_y2, 
      dm0_min, m_fit_min, mmin_y1, mmin_y2,
      m_y1_per, m_y2_per, m_y10_per, m_y20_per,
      mmean_y1c_weigh, mmean_y2c_weigh, mmean_y1c0_weigh, mmean_y2c0_weigh)
...
LoI_maize = pd.concat([maize_yield['lats'], maize_yield['lons'], maize_yield['growArea'], m_yield_y1,
                       m_y1_change, m_yield_y2, m_y2_change, m_c0], axis='columns')
...
utilities.create5minASCIIneg(LoI_maize,'m_y1_c0',params.asciiDir+'LoIMaizeYieldChange_0y1')
utilities.create5minASCIIneg(LoI_maize,'m_y2_c0',params.asciiDir+'LoIMaizeYieldChange_0y2')
