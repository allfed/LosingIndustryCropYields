...
crop_dict = {
    "maize": {"pesticides": "Corn", "yield": "MAIZ"},
...
crop_yield = pd.read_csv(
    params.geopandasDataDir
    + crop_dict[CROP]["yield"]
    + "CropYield"
    + postfix
    + "Filtered.csv"
)
...
pesticides = pd.read_csv(
    params.geopandasDataDir
    + crop_dict[CROP]["pesticides"]
    + "Pesticides"
    + postfix
    + "Filtered.csv"
)
fertilizer = pd.read_csv(
    params.geopandasDataDir + "Fertilizer" + postfix + "Filtered.csv"
)  # kg/m²
fertilizer_man = pd.read_csv(
    params.geopandasDataDir + "FertilizerManure" + postfix + "Filtered.csv"
)  # kg/km²
crop = pd.read_csv(params.geopandasDataDir + "FracCropArea" + postfix + "Filtered.csv")
if IRRIGATED:
    irr_rel = pd.read_csv(
        params.geopandasDataDir + "FracReliant" + postfix + "Filtered.csv"
    )
    irr_t = pd.read_csv(
        params.geopandasDataDir + "FracIrrigationArea" + postfix + "Filtered.csv"
    )
tillage = pd.read_csv(
    params.geopandasDataDir + "TillageAllCrops" + postfix + "Filtered.csv"
)
...
d0_elim.loc[
    d0_elim["n_fertilizer"] < 0, "n_fertilizer"
] = np.nan  # only 2304 left, so ffill
...
# n_manure values
d0_elim.loc[d0_elim["n_total"] < 0, "n_total"] = (
    d0_elim["n_fertilizer"] + d0_elim["n_manure"]
)
...
if IRRIGATED:
    dcrop_dum_elim = dcrop_d_elim.drop(
        ["270+days", "Temp_cool+Bor+Arctic", "L1_irr"], axis="columns"
    )
else:
    dcrop_dum_elim = dcrop_d_elim.drop(
        ["270+days", "Temp_cool+Bor+Arctic"], axis="columns"
    )
...
if IRRIGATED:
    dcrop_cor_elim = dcrop_fit_elim.drop(
        [
            "lat",
            "lon",
            "area",
...
if IRRIGATED:
    ...
    mod_elimg = smf.glm(
        formula="Y ~ n_total + mechanized + irrigation_tot + p_fertilizer + pesticides_H + \
    ...
    mod_elimg = smf.glm(
        formula="Y ~ n_total + p_fertilizer + pesticides_H + mechanized + \
              C(thz_class) + C(mst_class) + C(soil_class)",
        data=dcrop_fit_elim,
        family=sm.families.Gamma(link=sm.families.links.log),
    )
...
def downsample(highres_data, highres_prediction, scale):
    ...
    lowres["sumproduct_pred"] = highres_data.groupby(
        ["latbin", "lonbin"]
    ).sumproduct_pred.sum()
...
mn_drop = LoI_celim[
    ((LoI_celim["mechanized"].isna()) | (LoI_celim["pesticides_H"].isna()))
    & (LoI_celim["n_fertilizer"] < 0)
].index
LoI_celim_pn = LoI_celim.drop(mn_drop)

# replace remaining no data values in the fertilizer datasets with NaN and then fill them
LoI_celim_pn.loc[
    LoI_celim_pn["n_fertilizer"] < 0, "n_fertilizer"
] = np.nan  # only 2304 left, so ffill
LoI_celim_pn.loc[LoI_celim_pn["p_fertilizer"] < 0, "p_fertilizer"] = np.nan
LoI_celim_pn[["n_fertilizer", "p_fertilizer"]] = LoI_celim_pn[
    ["n_fertilizer", "p_fertilizer"]
].fillna(method="ffill")
# replace no data values in n_total with the sum of the newly filled n_fertilizer and the
# n_manure values
LoI_celim_pn.loc[LoI_celim_pn["n_total"] < 0, "n_total"] = (
    LoI_celim_pn["n_fertilizer"] + LoI_celim_pn["n_manure"]
)

# drop the nonsense values (99.9th percentile) in the n and p fertilizer columns
LoI_celim_pn = LoI_celim_pn.loc[
    LoI_celim_pn["n_fertilizer"] < d0_qt.iloc[12, 4]
]  # ~180
...
if IRRIGATED:
    LoI_celim["irr_LoI"] = LoI_celim["irrigation_tot"] * (
        1 - LoI_celim["irrigation_rel"]
    )
...
LoI_celim = pd.concat(
    [
        LoI_celim,
        LoI_celim_pn["n_fert_y1"],
        LoI_celim_pn["n_fert_y2"],
        LoI_celim_pn["p_fert_y1"],
        LoI_celim_pn["p_fert_y2"],
        LoI_celim_man["man_fert"],
        LoI_celimp["pest_y1"],
        LoI_celimp["pest_y2"],
    ],
    axis="columns",
...
mmean_y1_weigh = round(
    np.average(yield_y1, weights=LoI_celim["area"]), 2
)  # 3832.02kg/ha
...
mmean_y2_weigh = round(
    np.average(yield_y2, weights=LoI_celim["area"]), 2
)  # 2792.08kg/ha
...
def downsample_prediction_1col(highres_data, data_col, step):
    ...
    lowres = pd.DataFrame({})
    lowres["area"] = highres_data.groupby(["latbin", "lonbin"]).area.sum()
    lowres["sumproduct_" + data_col] = highres_data.groupby(["latbin", "lonbin"])[
        data_col
    ].sum()
...
if SAVE_DOWNSAMPLES:
    ...
    for dc in downsample_columns:
        ...
        utilities.createASCII(
            lowres, dc, params.asciiDir + crop_dict[CROP]["yield"] + "LoI_" + dc
        )
        print("saved " + dc)
else:
