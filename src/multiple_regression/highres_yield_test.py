...
# with z matrix
dwheat_fit_z = (
    dwheat_fit_elim.iloc[:, [3, 4, 5, 6, 7, 8, 9]].dropna().apply(stats.zscore)
)
w_stand = pd.concat(
    [dwheat_fit_z, dwheat_fit_elim.iloc[:, [10, 11, 12, 13, 14, 15]]], axis="columns"
)
...
dwheat_fit_n = (
    dwheat_fit_elim.iloc[:, [3, 4, 5, 6, 7, 8, 9]]
    / dwheat_fit_elim.iloc[:, [3, 4, 5, 6, 7, 8, 9]].max()
)
w_norm = pd.concat(
    [dwheat_fit_n, dwheat_fit_elim.iloc[:, [10, 11, 12, 13, 14, 15]]], axis="columns"
)
dwheat_fit_nt = (
    dwheat_fit_elim.iloc[:, [4, 5, 6, 7, 8, 9]]
    / dwheat_fit_elim.iloc[:, [4, 5, 6, 7, 8, 9]].max()
)
w_norm_t = pd.concat(
    [dwheat_fit_nt, dwheat_fit_elim.iloc[:, [3, 10, 11, 12, 13, 14, 15]]],
    axis="columns",
)
"""
...
w_mod_standt = smf.glm(
    formula="Y ~ n_total + p_fertilizer + pesticides_H ",
    data=w_stand,
    family=sm.families.Gamma(link=sm.families.links.log),
)
# normalized model
...
w_mod_normtp = smf.glm(
    formula="Y ~ n_total + irrigation_tot + mechanized + pesticides_H +  C(thz_class) + \
              C(mst_class) + C(soil_class)",
    data=w_norm_t,
    family=sm.families.Gamma(link=sm.families.links.log),
)
...
w_mod_stand0 = smf.glm(
    formula="Y ~ 1", data=w_stand, family=sm.families.Gamma(link=sm.families.links.log)
)
