...
soyb_yield = pd.read_pickle(
    params.inputDataDir + "SOYBCropYieldHighRes.pkl", compression="zip"
)
...
s_pesticides = pd.read_pickle(
    params.inputDataDir + "SoybeanPesticidesHighRes.pkl", compression="zip"
)
...
# n_manure values
ds0_elim.loc[ds0_elim["n_total"] < 0, "n_total"] = (
    ds0_elim["n_fertilizer"] + ds0_elim["n_manure"]
)

# calculate the 25th, 50th, 75th, 85th, 95th, 99th and 99.9th percentile
ds0_qt = ds0_elim.quantile([0.25, 0.5, 0.75, 0.85, 0.95, 0.99, 0.999])
...
#####Rename Columns##########
dus_mst_elim = dus_mst_elim.rename(
    columns={
        1: "LGP<120days",
        3: "120-180days",
        4: "180-225days",
...
dus_thz_elim = dus_thz_elim.rename(
    columns={
        1: "Trop_low",
        2: "Trop_high",
        3: "Sub-trop_warm",
...
dsoyb_d_elim = pd.concat(
    [ds0_elim, dus_mst_elim, dus_thz_elim, dus_soil_elim], axis="columns"
)
...
dsoyb_cor_elim = dsoyb_fit_elim.drop(
    [
        "lat",
        "lon",
        "area",
...
np.sqrt(
    mean_tweedie_deviance(dsoyb_fit_elim["Y"], s_fit_elimg.fittedvalues, power=2)
)  # 0.3828
...
plot_ss.axes[0] = sb.regplot(
    "Fitted_link",
    "resid_stud",
    data=s_elimg_infl_sample,
    lowess=True,
...
for i in abs_resid_top_3.index:
    plot_sr.axes[0].annotate(
        i, xy=(s_elimg_infl["GLM_fitted"][i], s_elimg_infl["resid_resp_abs"][i])
    )
...
for r, i in enumerate(abs_norm_resid_top_3):
    plot_sq.axes[0].annotate(
        i, xy=(np.flip(QQ.theoretical_quantiles, 0)[r], s_elimg_infl["resid_stud"][i])
    )
...
s_cook_infl = s_elimg_cook.loc[s_elimg_cook > (4 / (62237 - 21))].sort_values(
    ascending=False
)
...
sb.regplot(
    s_elimg_infl["hat_matrix"],
    s_elimg_infl["resid_stud"],
    scatter=False,
    ci=False,
    # lowess=True,
    line_kws={"color": "red", "lw": 1, "alpha": 0.8},
...
for i in leverage_top_3:
    plot_sc.axes[0].annotate(
        i, xy=(s_elimg_infl["hat_matrix"][i], s_elimg_infl["resid_stud"][i])
    )
...
sn_drop = LoI_selim[
    ((LoI_selim["mechanized"].isna()) | (LoI_selim["pesticides_H"].isna()))
    & (LoI_selim["n_fertilizer"] < 0)
].index
...
LoI_selim_pn[["n_fertilizer", "p_fertilizer"]] = LoI_selim_pn[
    ["n_fertilizer", "p_fertilizer"]
].fillna(method="ffill")
# replace no data values in n_total with the sum of the newly filled n_fertilizer and the
# n_manure values
LoI_selim_pn.loc[LoI_selim_pn["n_total"] < 0, "n_total"] = (
    LoI_selim_pn["n_fertilizer"] + LoI_selim_pn["n_manure"]
)
...
LoI_selim_pn["n_fert_y1"] = (s_ntot_new * LoI_selim_pn["n_ffrac"]) / LoI_selim_pn[
    "area"
]
...
# the distribution remains the same as before the catastrophe
LoI_selim_pn["p_fert_y1"] = (s_ptot_new * LoI_selim_pn["p_ffrac"]) / LoI_selim_pn[
    "area"
]
...
LoI_selim = pd.concat(
    [
        LoI_selim,
        LoI_selim_pn["n_fert_y1"],
        LoI_selim_pn["n_fert_y2"],
        LoI_selim_pn["p_fert_y1"],
        LoI_selim_pn["p_fert_y2"],
        LoI_selim_man["man_fert"],
        LoI_selimp["pest_y1"],
        LoI_selimp["pest_y2"],
    ],
    axis="columns",
...
LoI_s_year1 = LoI_s_year1.rename(
    columns={
...
smean_y1_weigh = round(
    np.average(s_yield_y1, weights=LoI_selim["area"]), 2
)  # 2257.13kg/ha
...
# change rate
smean_y1c_weigh = round(
    np.average(s_y1_change, weights=sw), 2
)  # -0.04 (~-4%) -0.1 (~10%)
...
# rename the columns according to the names used in the model formular
LoI_s_year2 = LoI_s_year2.rename(
    columns={
...
# calculate weighted mean, min and max of predicted yield (1) including postive change rates
smean_y2_weigh = round(
    np.average(s_yield_y2, weights=LoI_selim["area"]), 2
)  # 1600.52 kg/ha
...
smean_y2c_weigh = round(
    np.average(s_y2_change, weights=sw), 2
)  # -0.3 (~-30%) -0.36 (~36%)
...
LoI_soyb = LoI_soyb.rename(
    columns={0: "s_yield_y1", 1: "s_y1_change", 2: "s_yield_y2", 3: "s_y2_change"},
    errors="raise",
)
...
utilities.create5minASCIIneg(
    LoI_soyb, "s_y1_change", params.asciiDir + "LoISoybYieldChange_y1"
)
utilities.create5minASCIIneg(
    LoI_soyb, "s_yield_y1", params.asciiDir + "LoISoybYield_y1"
)
utilities.create5minASCIIneg(
    LoI_soyb, "s_y2_change", params.asciiDir + "LoISoybYieldChange_y2"
)
utilities.create5minASCIIneg(
    LoI_soyb, "s_yield_y2", params.asciiDir + "LoISoybYield_y2"
)
utilities.create5minASCIIneg(
    LoI_soyb, "s_y1_c0", params.asciiDir + "LoISoybYieldChange_0y1"
)
utilities.create5minASCIIneg(
    LoI_soyb, "s_y2_c0", params.asciiDir + "LoISoybYieldChange_0y2"
)
