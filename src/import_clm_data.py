'''
This code imports the netcdf files from Toon group clm dataset. The dataset has the nuclear event starting mid-may year 5. The model uses a nuclear winter scenario involving 150 Teragrams of soot from burning of cities.

To access the raw data, you need access to the ALLFED account in app.globus.com.
...
lats=np.array([])
lons=np.array([])
allMonths=['0005-01'] #just one month to keep it simple
print('params to actually import:')
print(params.clmVarNames)
print('')
for vn in params.clmVarNames:
	gridAllMonths=[]
	for month in allMonths:
		ds = nc.Dataset(params.rawClmDataLoc + month + '.nc')
		# uncomment below to see all the names of the different nc variables (which are specified by latitude and longitude)
		if(month==allMonths[0]):
			if(vn== params.clmVarNames[0]):
				print('All possible params:')
				print('###')
				for i in ds.variables.items():
					if(i[1].ndim>2):
						# uncomment section below to see some interesting variables for this project. Some of them are used.
						print(i[0])
						print(i[1].units)
						print(i[1].long_name)
						print('###')
