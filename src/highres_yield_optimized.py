...
m_pesticides=pd.read_csv(params.geopandasDataDir +crop_dict[CROP]['pesticides']+ 'Pesticides'+postfix+'Filtered.csv')
...
if(IRRIGATED):
    ...
    m_mod_elimg = smf.glm(formula='Y ~ n_total + mechanized + irrigation_tot + p_fertilizer + pesticides_H + \
              C(thz_class) + C(mst_class) + C(soil_class)', data=dmaize_fit_elim, 
               family=sm.families.Gamma(link=sm.families.links.log))
...
if(IRRIGATED):
    independent_columns=['p_fertilizer','n_total','pesticides_H','irrigation_rel','mechanized','thz_class','mst_class','soil_class','irrigation_tot']
else:
...
LoI_maize.to_csv(params.geopandasDataDir + "LoI"+crop_dict[CROP]['pesticides']+"Yield"+postfix+"Filtered.csv")
print('saved')