	...
	for c in crops:
		for b in bounds:
			...
			if(os.path.exists(fn)):
				...
				# pBinnedReoriented=np.fliplr(np.transpose(pBinned))
				pBinnedReoriented=np.flipud(pBinned)
				n_imported=n_imported+1

				#print crop if data for application of this pesticide exists 
				if(b=='L'):
					print('    '+c)
...
plotGrowArea=True
if(b=="L"):
	title=p+" Pesticide Application Rate, 2020, Lower Bound"
else:
	title=p+" Pesticide Application Rate, 2020, Upper Bound"
