...
result = np.full((nbins * len(lats), nbins * len(lons)), -9.0)
...
for c in crops:
    for b in bounds:
        ...
        for p in pesticides:
            ...
            if os.path.exists(fn):
                ...
                result[
                    start_lat_index : start_lat_index + len(pArr),
                    start_lon_index : start_lon_index + len(pArr[0]),
                ] = pArr
    ...
    if MAKE_GRID:
        ...
        Plotter.plotMap(
            grid, "total_L", title, label, "TotPesticidesByCropLow", plotGrowArea
        )
        plotGrowArea = True
        title = c + " Total Pesticide Application Rate, 2015, Upper Bound"
        label = "Application Rate (kg/ha/year)"
        Plotter.plotMap(
            grid, "total_H", title, label, "TotPesticidesByCropHigh", plotGrowArea
        )
        ...
        df.to_pickle(
            params.inputDataDir + c + "PesticidesHighRes.pkl", compression="zip"
        )
