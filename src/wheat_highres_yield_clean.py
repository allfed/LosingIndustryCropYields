...
'''
w_pesticides = pd.read_csv(params.geopandasDataDir +
                           'WheatPesticidesHighRes.csv')
fertilizer = pd.read_csv(params.geopandasDataDir +
                         'FertilizerHighRes.csv')  # kg/m²
fertilizer_man = pd.read_csv(
    params.geopandasDataDir + 'FertilizerManureHighRes.csv')  # kg/km²
...
man_new.rename(columns={'applied': 'applied_kgha',
               'produced': 'produced_kgha'}, inplace=True)
...
dw0_elim.loc[dw0_elim['n_fertilizer'] < 0,
             'n_fertilizer'] = np.nan  # only 2304 left, so ffill
...
# n_manure values
dw0_elim.loc[dw0_elim['n_total'] < 0,
             'n_total'] = dw0_elim['n_fertilizer'] + dw0_elim['n_manure']
...
nf_out = dw0_elim.loc[dw0_elim['n_fertilizer'] > dw0_qt.iloc[6, 4]]  # ~180
pf_out = dw0_elim.loc[dw0_elim['p_fertilizer'] > dw0_qt.iloc[6, 5]]  # ~34
nm_out = dw0_elim.loc[dw0_elim['n_manure'] > dw0_qt.iloc[5, 6]]  # ~11
# dm0_elim = dm0_elim.loc[dm0_elim['n_man_prod'] < dm0_qt.iloc[6,7]] #~44
nt_out = dw0_elim.loc[dw0_elim['n_total'] > dw0_qt.iloc[6, 8]]  # ~195
P_out = dw0_elim.loc[dw0_elim['pesticides_H'] > dw0_qt.iloc[6, 9]]  # ~11
...
dwheat_d_elim = pd.concat(
    [dw0_elim, duw_mst_elim, duw_thz_elim, duw_soil_elim], axis='columns')
...
# determine models
w_mod_elimn = smf.ols(formula=' Y ~ n_total + np.square(n_total) + p_fertilizer + pesticides_H + irrigation_tot + mechanized +  C(thz_class) + \
...
# Nullmodel
w_mod_elim0 = smf.glm(formula='Y ~ 1', data=dwheat_fit_elim,
                      family=sm.families.Gamma(link=sm.families.links.log))
# Fit models
...
#w_fit_elimg.pseudo_rsquared(kind='mcf')
#it'S on the statsmodel page but tells me that it's not an attribute of the results class
d2_tweedie_score(dwheat_fit_elim['Y'],
                 w_fit_elimg.fittedvalues, power=2)  # 0.3778
np.sqrt(mean_tweedie_deviance(
    dwheat_fit_elim['Y'], w_fit_elimg.fittedvalues, power=2))  # 0.5194
np.sqrt(mean_tweedie_deviance(
    dwheat_fit_elim['Y'], w_fit_elim0.fittedvalues, power=2))  # 0.5194


d2_tweedie_score(dwheat_fit_elim['Y'],
                 w_fit_elimn.fittedvalues, power=0)  # 0.4301
np.sqrt(mean_tweedie_deviance(
    dwheat_fit_elim['Y'], w_fit_elimn.fittedvalues, power=0))  # 1451.013
...
'''
##########RESIDUALS for the Normal distribution#############
...
w_stat_elimn = w_fit_elimn.get_influence()
# print(w_stat_elimg.summary_table()), there seems to be too much data

# store cook's distance in a variable
w_elimn_cook = pd.Series(w_stat_elimn.cooks_distance[0]).transpose()
w_elimn_cook = w_elimn_cook.rename("Cooks_d", errors="raise")
...
w_data_infl = {'Yield': dwheat_fit_elim['Y'],
               'GLM_fitted': w_fit_elimn.fittedvalues,
               # 'Fitted_link': w_fit_elimn.predict(w_fit_elim, linear=True),
               'resid_pear': w_fit_elimn.resid_pearson,
               'resid_stud': w_stat_elimn.resid_studentized,
               'resid_resp_abs': np.abs(w_fit_elimn.resid),
               'resid_stud_sqrt': np.sqrt(np.abs(w_stat_elimn.resid_studentized)),
               'hat_matrix': w_stat_elimn.hat_matrix_diag}
w_elimn_infl = pd.DataFrame(data=w_data_infl).reset_index()
w_elimn_infl = pd.concat([w_elimn_infl, w_elimn_cook], axis='columns')


# take a sample of the influence dataframe to plot the lowess line
w_elimn_infl_sample = w_elimn_infl.sample(frac=0.1, random_state=2705)
...
plot_ws.axes[0] = sb.regplot('Fitted_link', 'resid_stud', data=w_elimn_infl_sample,
                             lowess=True,
...
plot_wr.axes[0] = sb.residplot('GLM_fitted', 'Yield', data=w_elimn_infl_sample,
                               lowess=True,
                               scatter_kws={'alpha': 0.5},
                               line_kws={'color': 'red', 'lw': 1, 'alpha': 0.8})
...
# annotations
abs_resid = w_elimn_infl_sample['resid_resp_abs'].sort_values(ascending=False)
abs_resid_top_3 = abs_resid[:3]

for i in abs_resid_top_3.index:
    plot_wr.axes[0].annotate(i,
                             xy=(w_elimn_infl_sample['GLM_fitted'][i],
                                 w_elimn_infl_sample['resid_resp_abs'][i]))

###############QQ-Plot########################

QQ = ProbPlot(w_elimn_infl['resid_stud'])
...
# annotations
abs_norm_resid = np.flip(np.argsort(np.abs(w_elimn_infl['resid_stud'])), 0)
abs_norm_resid_top_3 = abs_norm_resid[:3]

for r, i in enumerate(abs_norm_resid_top_3):
    plot_wq.axes[0].annotate(i,
                             xy=(np.flip(QQ.theoretical_quantiles, 0)[r],
                                 w_elimn_infl['resid_stud'][i]))
...
w_cook_sort = w_elimn_cook.sort_values(ascending=False)
w_cook_sort.mean()
# select all Cook's distance values which are greater than 4/n (n=number of datapoints)
w_cook_infl = w_elimn_cook.loc[w_elimn_cook >
                               (4/273772)].sort_values(ascending=False)
...
plt.scatter(w_elimn_cook.index, w_elimn_cook)
plt.ylim(0, 0.01)

ax = sb.boxplot(x=w_elimn_infl["Cooks_d"])
...
#0.0000149
# select all Cook's distance values which are greater than 4/n (n=number of datapoints)
w_cook_infl = w_elimg_cook.loc[w_elimg_cook >
                               (4/(168227-21))].sort_values(ascending=False)
#10642 remaining outliers
w_cook_infl.mean()
#0.000198 only influential points
#0.00000253 without the influential points
...
ax = sb.boxplot(x=w_elimg_infl["Cooks_d"])
...
plt.scatter(w_elimg_infl_sample['hat_matrix'],
            w_elimg_infl_sample['resid_stud'], alpha=0.5)
...
# replace remaining no data values in the fertilizer datasets with NaN and then fill them
LoI_welim_pn.loc[LoI_welim_pn['n_fertilizer'] < 0,
                 'n_fertilizer'] = np.nan  # only 2304 left, so ffill
LoI_welim_pn.loc[LoI_welim_pn['p_fertilizer'] < 0, 'p_fertilizer'] = np.nan
LoI_welim_pn[['n_fertilizer', 'p_fertilizer']] = LoI_welim_pn[[
    'n_fertilizer', 'p_fertilizer']].fillna(method='ffill')
# replace no data values in n_total with the sum of the newly filled n_fertilizer and the
# n_manure values
LoI_welim_pn.loc[LoI_welim_pn['n_total'] < 0,
                 'n_total'] = LoI_welim_pn['n_fertilizer'] + LoI_welim_pn['n_manure']

# drop the nonsense values (99.9th percentile) in the n and p fertilizer columns
# ~180
LoI_welim_pn = LoI_welim_pn.loc[LoI_welim_pn['n_fertilizer']
                                < dw0_qt.iloc[6, 4]]
# ~34
LoI_welim_pn = LoI_welim_pn.loc[LoI_welim_pn['p_fertilizer']
                                < dw0_qt.iloc[6, 5]]
...
LoI_welim_pn['n_fert_y1'] = (
    w_ntot_new * LoI_welim_pn['n_ffrac']) / LoI_welim_pn['area']
...
# the distribution remains the same as before the catastrophe
LoI_welim_pn['p_fert_y1'] = (
    w_ptot_new * LoI_welim_pn['p_ffrac']) / LoI_welim_pn['area']
...
LoI_welim_man = LoI_welim.loc[LoI_welim['n_manure'] < dw0_qt.iloc[5, 6]]  # ~11
...
# current value (7.4) is taken from Dave's paper
LoI_welim_man['labor'] = LoI_welim_man['area']/5
...
LoI_welim_man['man_fert'] = (
    cow_excr * LoI_welim_man['labor']) / LoI_welim_man['area']
...
LoI_welimp = LoI_welimp.loc[LoI_welimp['pesticides_H']
                            < dw0_qt.iloc[6, 9]]  # ~11
...
LoI_welim['irr_LoI'] = LoI_welim['irrigation_tot'] * \
    (1 - LoI_welim['irrigation_rel'])
...
#dw0_qt.reset_index(inplace=True, drop=True)
LoI_welim = LoI_welim.loc[LoI_welim['Y'] < dw0_qt.iloc[6, 3]]  # ~12500
# dw0_elim = dw0_elim.loc[dw0_elim['n_man_prod'] < dw0_qt.iloc[7,7]] #~44
LoI_welim = LoI_welim.loc[LoI_welim['n_total'] < dw0_qt.iloc[6, 8]]  # ~195
...
s1 = w_y1_change.loc[w_y1_change > 0]  # 34057 cells
ax = sb.boxplot(x=s1)
...
w_t1_change = ((w_y1_y0-wheat_kgha)/wheat_kgha).dropna()
test = w_t1_change.loc[w_t1_change>0]
...
s2 = w_y2_change.loc[w_y2_change > 0]  # 30967 cells
...
round(w_y1_change.quantile(
    [.01, .05, .1, .2, .25, .3, .4, .5, .6, .7, .75, .8, .9, .95, .99]), 2)
round(w_y2_change.quantile(
    [.01, .05, .1, .2, .25, .3, .4, .5, .6, .7, .75, .8, .9, .95, .99]), 2)
...
wmean_y1_weigh/dw0_mean  # ~72.2% of current average yield
...
# Year 2 yield
wmean_y2_weigh/dw0_mean  # 57.4% of current average yield
...
print(dw0_mean, wmean_y1_weigh, wmean_y2_weigh,
      dw0_prod, w_y1_prod, w_y2_prod,
      dw0_max, wmax_y1, wmax_y2, w_fit_max,
      dw0_min, wmin_y1, wmin_y2, w_fit_min,
      wmean_y1c_weigh, wmean_y2c_weigh,
      wmean_y1c0_weigh, wmean_y2c0_weigh)
